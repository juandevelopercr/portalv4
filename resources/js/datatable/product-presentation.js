...
$(function () {
  $(document).ready(function () {
    ...
    var dt_table = $('.datatables-product-presentation'),
      select2 = $('.select2'),
    ...
    if (dt_table.length) {
      var dt_user = dt_table.DataTable({
        ...
        serverSide: true,
        ajax: {
          url: baseUrl + 'products-presentations/ajax/list'
        },
        columns: [
        ...
        columnDefs: [
...

          {
            ...
            render: function (data, type, full, meta) {
              ...
              if ($user_img) {
                ...
                var $output =
                  '<img src="' + baseUrl + 'storage/assets/img/products-presentations/' + $user_img + '" alt="Product" class="rounded">';
              } else {
                // For Avatar badge
          ...
          },
          {
            ...
            render: function (data, type, full, meta) {
              ...
              return '<span class="user-email">' + $product + '</span>';
            }
          },
          {
            // unmit type
            targets: 5,
            render: function (data, type, full, meta) {
              var $unittype = full['unit'];

              return '<span class="user-email">' + $unittype + '</span>';
            }
          },
          {
            // active
          ...
          },
          {
            ...
            render: function (data, type, full, meta) {
              return (
                ...
                // Botón de editar
                `<a href="/products-presentations/${full['id']}/edit" class="btn btn-sm btn-icon edit-record">` +
        ...
        buttons: [
          ...
          },
          {
            ...
            action: function (e, dt, node, config) {
              window.location.href = '/products-presentations/create'; // Cambia '/ruta/del/formulario' por la ruta que quieras usar
            }
          }
        ],
    ...
    $(document).on('click', '.delete-record', function () {
      ...
      Swal.fire({
        ...
        if (result.value) {
          // delete the data
          $.ajax({
            url: `${baseUrl}` + `products-presentations/${user_id}`, // Asegúrate de que `baseUrl` esté correctamente definido
            type: 'DELETE',
            dataType: 'json',
    ...
    $(document).on('click', '.edit-record', function () {
      var user_id = $(this).data('id');
      window.location.href = '/products-presentations/' + user_id;
    });

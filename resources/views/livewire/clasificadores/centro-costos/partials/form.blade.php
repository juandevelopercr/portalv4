...
<div class="card mb-6">
  <form wire:submit.prevent="{{ $action == 'edit' ? 'update' : 'store' }}" class="card-body">
    ...
    <div class="row g-6">
      ...
      <div class="col-md-4 fv-plugins-icon-container">
        <label class="form-label" for="mcorto">{{ __('Descripción corta') }}</label>
        <div class="input-group input-group-merge has-validation">
          <span class="input-group-text"><i class="bx bx-font"></i></span>
          <input type="text" wire:model="mcorto" id="mcorto"
            class="form-control @error('mcorto') is-invalid @enderror" placeholder="{{ __('Descripción corta') }}">
        </div>
        @error('mcorto')
        <div class="text-danger mt-1">{{ $message }}</div>
        @enderror
      ...
      <div class="col-md-4 fv-plugins-icon-container">
        <label class="form-label" for="codcont">{{ __('Detalle') }}</label>
        <div class="input-group input-group-merge has-validation">
          <span class="input-group-text"><i class="bx bx-font"></i></span>
          <input type="text" wire:model="codcont" id="codcont"
            class="form-control @error('codcont') is-invalid @enderror" placeholder="{{ __('Detalle') }}">
        </div>
        @error('codcont')
        <div class="text-danger mt-1">{{ $message }}</div>
        @enderror
...
<script>
  (function() {
    ...
    */
    ...
    Livewire.on('reinitFormCentrocostoControls', () => {
      console.log('Reinicializando controles después de Livewire update reinitFormCentrocostoControls');
      setTimeout(() => {
        initializeSelect2();

...
<script>
  ...
  function initAllSelects() {
    ...
    // Inicializar selects de departamentos
    $('.department-select').each(function() {
      ...
      if ($select.data('select2')) {
        console.log(`Select2 ya inicializado para departamento (${role}), actualizando...`);
      ...
      $select.select2({
        ...
        console.log(`Cambio en departamento (${role}): ${value}`);
        @this.set(`roleAssignments.${role}.department_id`, value);
        updateBanksForRole(role, value);
      });
    });

    // Inicializar selects de bancos
    $('.bank-select').each(function() {
      ...
      // Si ya está inicializado, solo actualizar
      if ($select.data('select2')) {
        console.log(`Select2 ya inicializado para bancos (${role}), actualizando...`);
        $select.trigger('change.select2');
        return;
      }

      $select.select2({
        placeholder: 'Seleccione bancos',
        width: '100%'
      }).on('change', function() {
        const values = $(this).val() || [];
        console.log(`Cambio en bancos (${role}):`, values);
        @this.set(`roleAssignments.${role}.banks`, values);
      });
    });
  }
  ...
  document.addEventListener('livewire:initialized', () => {
    console.log("Evento: livewire:initialized");
    initAllSelects();
  });

  // Escuchar evento personalizado para reinicializar
  Livewire.on('reinitFormControls', () => {
    console.log("Evento personalizado: reinitFormControls");
    initAllSelects();
  });

  // Función para actualizar bancos
  function updateBanksForRole(role, departmentId) {
    console.log(`Actualizando bancos para rol: ${role}, departamento: ${departmentId}`);

    @this.call('updateBanksForRole', role, departmentId, (banks) => {
      console.log(`Bancos recibidos para ${role}:`, banks);

      const $select = $(`#bankSelect-${role}`);
      const currentValues = $select.val() || [];

      // Actualizar opciones manteniendo selecciones existentes
      $select.empty();
      banks.forEach(bank => {
        const isSelected = currentValues.includes(bank.id);
        $select.append(new Option(bank.name, bank.id, false, isSelected));
      });
  ...
  // Escuchar evento retrasado para reinicialización
  window.addEventListener('reinit-form-controls-delayed', () => {
    console.log("Evento retrasado para reinicialización");
    setTimeout(() => {
      Livewire.dispatch('reinitFormControls');
    }, 300);
  });
</script>
@endscript

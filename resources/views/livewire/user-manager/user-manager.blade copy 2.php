...
<script>
  $(document).ready(function() {
    ...
    window.initSelects = function() {
      // Inicializar selects de roles
      $('.role-select.select2').each(function() {
        const $select = $(this);
      ...
      $('.department-select.select2').each(function() {
        const $select = $(this);
        const index = $select.data('index');
        ...
        $select.select2({
          ...
          window.updateBanksForAssignment(index, value);
        });
      });

      // Inicializar selects de bancos
      $('.bank-select.select2').each(function() {
        const $select = $(this);
        const index = $select.data('index');
        ...
        $select.select2({
          ...
          @this.set(`roleAssignments.${index}.banks`, values || []);
        });
      });
    };

    window.updateBanksForAssignment = function(index, departmentId) {
      @this.call('updateBanksForRole', index, departmentId, (banks) => {
        const $select = $(`.bank-select[data-index="${index}"]`);
        ...
        banks.forEach(bank => {
          $select.append(new Option(
            ...
            bank.id,
            false,
            currentSelections.includes(bank.id.toString())
          ));
        });
    ...
    document.addEventListener('livewire:initialized', () => {
      console.log("Ejecutó el livewire:initialized");
      setTimeout(window.initSelects, 200);
    });

    // Manejar eventos de Livewire
    Livewire.on('reinitFormControls', () => {
      console.log("Ejecutó el reinitFormControls");
      setTimeout(window.initSelects, 200);
    });

    Livewire.on('assignmentAdded', () => {
      console.log("Ejecutó el assignmentAdded");
      setTimeout(window.initSelects, 200);
    });

    // Actualizar después de cambios en el DOM
    document.addEventListener('livewire:update', () => {
      console.log("Ejecutó el livewire:update");
      setTimeout(window.initSelects, 200);
    });
  });
</script>
...
<script>
  document.addEventListener('livewire:initialized', () => {
    console.log("Se ejecuta livewire:initialized");
    initSelects();
  });

  Livewire.on('reinitFormControls', postId => {
  ...
  document.addEventListener('livewire:update', () => {
    console.log("Se ejecuta livewire:update");
    initSelects();
  });
  ...
  function updateBanksForRole(role, departmentId) {
    // Obtener bancos para el nuevo departamento
    @this.call('updateBanksForRole', role, departmentId, (banks) => {
      ...
      $select.empty();
      banks.forEach(bank => {
        $select.append(new Option(bank.name, bank.id));
      });

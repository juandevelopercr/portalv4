<div class="card">
  <h4 class="card-header pb-0 text-md-start text-center ms-n2">{{ __('Reporte de Comisiones') }}</h4>
        ...
        <div class="row g-6">
          ...
          <div class="col-md-3 fv-plugins-icon-container">
            <div class="form-check form-switch ms-2 my-7">
              <input type="checkbox" class="form-check-input" id="filter_pagar" wire:model.live="filter_pagar" {{ $filter_pagar==1
                ? 'checked' : '' }} />

              <label for="future-billing" class="switch-label">{{ __('Pagar Comisión') }}</label>
            </div>
          </div>
        ...
        if (element) {
          ...
          $(`#${id}`).on('change', function() {
            ...
            if (newValue !== livewireValue) {
              ...
              const specificIds = ['filter_department','filter_abogado', 'filter_currency', 'filter_type']; // Lista de IDs específicos

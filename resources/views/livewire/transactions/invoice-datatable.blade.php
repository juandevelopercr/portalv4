...
<div wire:key="invoice-main-container">
<div x-data="{ action: @entangle('action') }">
    <div :class="{ 'd-none': action !== 'list' }" wire:key="invoice-list-container">
                ...
                <div class="card-datatable table-responsive">
                  <table class="table table-sm mb-0 border-top table-hover dataTable no-footer" id="transaction-table" style="width: 100%;">
                    ...
                    <tbody>
                      ...
                      <tr wire:key='invoice-row-{{ $record->id }}' class="{{ $loop->odd ? 'odd' : 'even' }}">
    ...
    <div :class="{ 'd-none': action !== 'create' && action !== 'edit' }" wire:key="invoice-form-container">
        @include('livewire.transactions.partials.form_invoice')
    </div>

    {{-- Área de Componentes Auxiliares Fijos para Estabilidad del DOM --}}
    <div wire:key="invoice-auxiliary-components">
        <livewire:modals.caby-modal wire:key="invoice-caby-modal" />
        ...
        ], 'invoice-datatable-config')

        @can("export-electronicinvoices")
            <div class="d-none" wire:key="invoice-export-container">
                <livewire:transactions.transaction-datatable-export exportType="invoice" wire:key="invoice-export-component" />
            </div>
        @endcan
    </div>
</div>

...
<div wire:key="recibo-pago-main-container">
<div x-data="{ action: @entangle('action') }">
    <div :class="{ 'd-none': action !== 'list' }" wire:key="recibo-pago-list-container">
        <!-- DataTable with Buttons -->
        <div class="card">
          <h4 class="card-header pb-0 text-md-start text-center ms-n2">{{ __('Recibo de pago electrónico') }}</h4>
            ...
            <div class="dataTables_wrapper dt-bootstrap5 no-footer">
              ...
              @can("view-proformas")
                <div class="card-datatable table-responsive">
                  <table class="table table-sm mb-0 border-top table-hover dataTable no-footer" id="recibo-table" style="width: 100%;">
                    <thead>
                      <tr>
                    ...
                    <tbody>
                      ...
                      <tr wire:key='recibo-pago-row-{{ $record->id }}' class="{{ $loop->odd ? 'odd' : 'even' }} {{ $record->department_id != 1 ? 'table-info' : '' }}">
    ...
    <div :class="{ 'd-none': action !== 'create' && action !== 'edit' }" wire:key="recibo-pago-form-container">
        @include('livewire.transactions.partials.form_recibo')
    </div>

    {{-- Área de Componentes Auxiliares Fijos para Estabilidad del DOM --}}
    <div wire:key="recibo-pago-auxiliary-components">
        <livewire:modals.caby-modal wire:key="recibo-pago-caby-modal" />
        ...
        ], 'recibo-pago-send-email-modal')

        @livewire('components.datatable-settings', [
            'datatableName' => 'recibo-pago-datatable',
            'availableColumns' => $this->columns,
            'perPage' => $this->perPage,
        ], 'recibo-datatable-config')

        @can("export-proformas")
            <div class="d-none" wire:key="recibo-pago-export-container">
                <livewire:transactions.transaction-datatable-export exportType="recibo-pago" wire:key="recibo-pago-export-component" />
            </div>
        @endcan
    </div>
</div>
...
<script>
  function printTable() {
      const printContent = document.getElementById('recibo-table').innerHTML;

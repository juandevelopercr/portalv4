...
<div wire:key="factura-compra-main-container">
<div x-data="{ action: @entangle('action') }">
    <div :class="{ 'd-none': action !== 'list' }" wire:key="factura-compra-list-container">
        <!-- DataTable with Buttons -->
        <div class="card">
          <h4 class="card-header pb-0 text-md-start text-center ms-n2">{{ __('Facturas electrónicas de compras') }}</h4>
                ...
                <div class="card-datatable table-responsive">
                  <table class="table table-sm mb-0 border-top table-hover dataTable no-footer" id="transaction-table" style="width: 100%;">
                    ...
                    <tbody>
                      ...
                      <tr wire:key='factura-compra-row-{{ $record->id }}' class="{{ $loop->odd ? 'odd' : 'even' }} {{ $record->department_id != 1 ? 'table-info' : '' }}">
    ...
    <div :class="{ 'd-none': action !== 'create' && action !== 'edit' }" wire:key="factura-compra-form-container">
        @include('livewire.transactions.partials.form_factura_compra')
    </div>

    {{-- Área de Componentes Auxiliares Fijos para Estabilidad del DOM --}}
    <div wire:key="factura-compra-auxiliary-components">
        <livewire:modals.caby-modal wire:key="factura-compra-caby-modal" />

        @livewire('transactions.send-email-modal', [
            'documentType' => 'FEC',
            'canview'   => auth()->user()->can('view-documento-facturascompras'),
            'cancreate' => auth()->user()->can('create-documento-facturascompras'),
            'canedit'   => auth()->user()->can('edit-documento-facturascompras'),
            'candelete' => auth()->user()->can('delete-documento-facturascompras'),
            'canexport' => auth()->user()->can('export-documento-facturascompras'),
        ], 'factura-compra-send-email-modal')

        @livewire('components.datatable-settings', [
            'datatableName' => 'factura-compra-datatable',
            'availableColumns' => $this->columns,
            'perPage' => $this->perPage,
        ], 'factura-compra-datatable-config')

        @can("export-facturascompras")
            <div class="d-none" wire:key="factura-compra-export-container">
                <livewire:transactions.transaction-datatable-export exportType="factura-compra" wire:key="factura-compra-export-component" />
            </div>
        @endcan
    </div>
</div>

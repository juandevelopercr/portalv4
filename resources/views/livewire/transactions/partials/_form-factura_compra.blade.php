<form wire:submit.prevent="{{ $action == 'edit' ? 'update' : 'store' }}" class="card-body">
  ...
  <div class="row g-6">
    ...
    <div class="col-md-6 fv-plugins-icon-container">
      <label class="form-label" for="customer_name">{{ __('Supplier') }}</label>
      <div class="input-group input-group-merge has-validation">
        <span class="input-group-text">
          <i class="bx bx-user"></i>
        </span>
        <input type="text" wire:model="customer_name" id="customer_name" readonly
          class="form-control @error('customer_name') is-invalid @enderror" placeholder="{{ __('Supplier') }}">
        <!-- Botón con icono -->
    ...
    <div class="col-md-3 select2-primary fv-plugins-icon-container">
      <label class="form-label" for="contact_economic_activity_id">{{ __('Supplier Economic Activity') }}</label>
      <div wire:ignore>
  ...
  <br>
  <h6 class="mt-4"><span class="badge bg-primary">2. {{ __('Payment Information') }}</span></h6>

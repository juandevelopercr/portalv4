  ...
  <div class="col-xl">
    <div class="card mb-6">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0">
          @if ($this->proforma_no)
          {{ __('Update Factura compra') }}: No. {{ $this->proforma_no }}
          @else
            ...
            <div class="tab-content">
              <div class="tab-pane fade show @if ($this->activeTab == 'invoice') show active @endif"
                  id="navs-justified-home" role="tabpanel">
                    @include('livewire.transactions.partials._form-factura_compra')
              </div>
...
<div wire:key="factura-compra-customer-modal-wrapper" x-show="$wire.modalCustomerOpen" style="display:none;">
  <div id="customer-modal" class="modal fade show d-block" style="background-color: rgba(0, 0, 0, 0.5);" tabindex="-1">
    <div class="modal-dialog modal-xl">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">{{ __('Select Supplier') }}</h5>
          <button type="button" class="btn-close" wire:click="closeCustomerModal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          @livewire('contacts.contact-manager', ['enabledSelectedValue' => true, 'type' => 'supplier'], key('factura-compra-contact-manager-modal'))
        </div>

...
<div wire:key="digital-debit-note-main-container">
<div x-data="{ action: @entangle('action') }">
    <div :class="{ 'd-none': action !== 'list' }" wire:key="digital-debit-note-list-container">
        <!-- DataTable with Buttons -->
        <div class="card">
          <h4 class="card-header pb-0 text-md-start text-center ms-n2">{{ __('Notas de débito digitales') }}</h4>
            ...
            <div class="dataTables_wrapper dt-bootstrap5 no-footer">
              ...
              @can("view-proformas")
                <div class="card-datatable table-responsive">
                  <table class="table table-sm mb-0 border-top table-hover dataTable no-footer" id="digital-debit-note-table" style="width: 100%;">
                    <thead>
                      <tr>
                    ...
                    <tbody>
                      ...
                      <tr wire:key='digital-debit-note-row-{{ $record->id }}' class="{{ $loop->odd ? 'odd' : 'even' }}">
    ...
    <div :class="{ 'd-none': action !== 'create' && action !== 'edit' }" wire:key="digital-debit-note-form-container">
        @include('livewire.transactions.partials.form_digital_debit_note')
    </div>

    {{-- Área de Componentes Auxiliares Fijos para Estabilidad del DOM --}}
    <div wire:key="digital-debit-note-auxiliary-components">
        <livewire:modals.caby-modal wire:key="digital-debit-note-caby-modal" />
        ...
        ], 'digital-debit-note-send-email-modal')

        @livewire('components.datatable-settings', [
            'datatableName' => 'digital-debit-note-datatable',
            'availableColumns' => $this->columns,
            'perPage' => $this->perPage,
        ], 'digital-debit-note-datatable-config')

        @can("export-proformas")
            <div class="d-none" wire:key="digital-debit-note-export-container">
                <livewire:transactions.transaction-datatable-export exportType="digital-debit-note" wire:key="digital-debit-note-export-component" />
            </div>
        @endcan
    </div>
</div>
...
<script>
  function printTable() {
      const printContent = document.getElementById('digital-debit-note-table').innerHTML;

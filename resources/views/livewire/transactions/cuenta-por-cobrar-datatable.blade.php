...
<div wire:key="cuenta-cobrar-main-container">
<div x-data="{ action: @entangle('action') }">
    <div :class="{ 'd-none': action !== 'list' }" wire:key="cuenta-cobrar-list-container">
                ...
                <div class="card-datatable table-responsive">
                  <table class="table table-sm mb-0 border-top table-hover dataTable no-footer" id="transaction-table" style="width: 100%;">
                    ...
                    <tbody>
                      ...
                      <tr wire:key='cuenta-cobrar-row-{{ $record->id }}{{ $counter }}' class="{{ $loop->odd ? 'odd' : 'even' }}">
    ...
    {{-- This datatable has no create/edit form; payments use a modal component --}}

    {{-- Área de Componentes Auxiliares Fijos para Estabilidad del DOM --}}
    <div wire:key="cuenta-cobrar-auxiliary-components">
        <livewire:modals.caby-modal wire:key="cuenta-cobrar-caby-modal" />
        ...
        ], 'cuenta-cobrar-send-email-modal')

        @livewire('components.datatable-settings', [
            'datatableName' => 'cuentas-cobrar-datatable',
            'availableColumns' => $this->columns,
            'perPage' => $this->perPage,
        ], 'cuentas-cobrar-datatable-config')

        @can("export-cuentasCobrar")
            <div class="d-none" wire:key="cuenta-cobrar-export-container">
                <livewire:transactions.transaction-datatable-export exportType="cuenta-por-cobrar" wire:key="cuenta-cobrar-export-component" />
            </div>
        @endcan
    </div>
</div>

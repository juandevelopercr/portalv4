...
<div wire:key="debit-note-main-container">
<div x-data="{ action: @entangle('action') }">
    <div :class="{ 'd-none': action !== 'list' }" wire:key="debit-note-list-container">
        <!-- DataTable with Buttons -->
        <div class="card">
          <h4 class="card-header pb-0 text-md-start text-center ms-n2">{{ __('Notas de débito electrónicas') }}</h4>
          <div class="card-datatable text-nowrap">
            <div class="dataTables_wrapper dt-bootstrap5 no-footer">
              ...
              @can("view-notasdebitoelectronicas")
                <div class="card-datatable table-responsive">
                  <table class="table table-sm mb-0 border-top table-hover dataTable no-footer" id="debit-note-table" style="width: 100%;">
                    <thead>
                      <tr>
                    ...
                    <tbody>
                      ...
                      <tr wire:key='debit-note-row-{{ $record->id }}' class="{{ $loop->odd ? 'odd' : 'even' }}">
    ...
    <div :class="{ 'd-none': action !== 'create' && action !== 'edit' }" wire:key="debit-note-form-container">
        @include('livewire.transactions.partials.form_debit_note')
    </div>

    {{-- Área de Componentes Auxiliares Fijos para Estabilidad del DOM --}}
    <div wire:key="debit-note-auxiliary-components">
        <livewire:modals.caby-modal wire:key="debit-note-caby-modal" />
        ...
        ], 'debit-note-send-email-modal')

        @livewire('components.datatable-settings', [
            'datatableName' => 'debit-note-datatable',
            'availableColumns' => $this->columns,
            'perPage' => $this->perPage,
        ], 'debit-note-datatable-config')

        @can("export-notasdebitoelectronicas")
            <div class="d-none" wire:key="debit-note-export-container">
                <livewire:transactions.transaction-datatable-export exportType="debit-note" wire:key="debit-note-export-component" />
            </div>
        @endcan
    </div>
</div>
...
<script>
  function printTable() {
      const printContent = document.getElementById('debit-note-table').innerHTML;

...
<div wire:key="digital-credit-note-main-container">
<div x-data="{ action: @entangle('action') }">
    <div :class="{ 'd-none': action !== 'list' }" wire:key="digital-credit-note-list-container">
                ...
                <div class="card-datatable table-responsive">
                  <table class="table table-sm mb-0 border-top table-hover dataTable no-footer" id="nota-credito-digital-table" style="width: 100%;">
                    ...
                    <tbody>
                      ...
                      <tr wire:key='digital-credit-note-row-{{ $record->id }}' class="{{ $loop->odd ? 'odd' : 'even' }}">
    ...
    <div :class="{ 'd-none': action !== 'create' && action !== 'edit' }" wire:key="digital-credit-note-form-container">
        @include('livewire.transactions.partials.form_digital_credit_note')
    </div>

    {{-- Área de Componentes Auxiliares Fijos para Estabilidad del DOM --}}
    <div wire:key="digital-credit-note-auxiliary-components">
        <livewire:modals.caby-modal wire:key="digital-credit-note-caby-modal" />
        ...
        ], 'digital-credit-note-send-email-modal')

        @livewire('components.datatable-settings', [
            'datatableName' => 'nota-credito-digital-datatable',
            'availableColumns' => $this->columns,
            'perPage' => $this->perPage,
        ], 'nota-credito-digital-datatable-config')

        @can("export-proformas")
            <div class="d-none" wire:key="digital-credit-note-export-container">
                <livewire:transactions.transaction-datatable-export exportType="digital-credit-note" wire:key="digital-credit-note-export-component" />
            </div>
        @endcan
    </div>
</div>

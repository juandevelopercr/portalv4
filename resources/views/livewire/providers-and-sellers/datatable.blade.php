<div>
  @if($action == 'list')
    ...
    <div class="card">
      <h4 class="card-header pb-0 text-md-start text-center ms-n2">{{ __('Proveedores y vendedores') }}</h4>
            ...
            <div class="col-md-10">
              <div
                ...
                <div class="dt-buttons btn-group flex-wrap">
                  <!-- Dropdown with icon -->
                  @can("create-providerandseller")
                    @include('livewire.includes.button-create')
                  @endcan

                  @can("edit-providerandseller")
                    @include('livewire.includes.button-edit')
                  @endcan

                  @can("delete-providerandseller")
                    @include('livewire.includes.button-delete', ['textButton'=>null])
                  @endcan

                  @can('export-providerandseller')
                    <livewire:provider-and-sellers.provider-and-seller-datatable-export />
                  @endcan
                  ...
                  @livewire('components.datatable-settings', [
                    'datatableName' => 'provider-and-seller-datatable',
                    'availableColumns' => $this->columns,
                    'perPage' => $this->perPage,
                  ],
                  key('provider-and-seller-datatable-config'))
  ...
  @if($action == 'create' || $action == 'edit')
    @include('livewire.providers-and-sellers.partials.form')
  @endif
</div>

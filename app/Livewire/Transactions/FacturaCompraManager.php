...
class FacturaCompraManager extends TransactionManager
{
  ...
  public function refresDatatable()
  {
    $config = DataTableConfig::where('user_id', Auth::id())
      ->where('datatable_name', 'factura-compra-datatable')
      ->first();
  ...
  public function getDefaultColumns(): array
  {
    $this->defaultColumns = [
      ...
      [
        ...
        'columnAlign' => '',
        'columnClass' => '',
        'function' => 'getFacturacompraHtmlColumnAction',
        'parameters' => [],
        'sumary' => '',
        'openHtmlTab' => '',
        'closeHtmlTab' => '',
  ...
  public function render()
  {
    ...
    return view('livewire.transactions.factura-compra-datatable', [
      'records' => $records,
    ]);
  }
  ...
  protected function rules()
  {
    $rules = [
      ...
      'RefNumero'             => 'nullable|string|max:50',
      'RefFechaEmision'       => 'required|date',
      'RefCodigo'             => 'nullable|string|min:2|max:2',
  ...
  public function store()
  {
    ...
    $this->document_type = 'PRC'; // Proforma de compra

    $this->RefTipoDoc = '16';
    $this->RefNumero = '123456';
    $this->RefFechaEmision = Carbon::now('America/Costa_Rica')->format('Y-m-d H:i:s');
    $this->RefCodigo = '04';
    $this->RefRazon = 'Compra';
  ...
  public function facturar($recordId)
  {
    ...
    // Validación
    $msgs = Helpers::validateFacturaCompraToConvertInvoice($record);
  ...
  private function enviarAHacienda($transaction)
  {
    try {
      ...
      if ($result['error'] == 0) {
        ...
        $this->dispatch('show-notification', [
          'type' => 'warning',
          'message' => __('Factura de compra guardada pero Hacienda la rechazó: ') . $result['mensaje'],
        ]);
      }
    } catch (\Exception $e) {
      Log::error('Error enviando factura de compra a Hacienda', ['error' => $e->getMessage(), 'id' => $transaction->id]);
      $this->dispatch('show-notification', [
        'type' => 'warning',
        'message' => __('Factura de compra guardada pero falló el envío a Hacienda: ') . $e->getMessage(),
      ]);
    }
  }
  ...
  public function beforeSendHacienda($id, $proformaNo)
  {
    $this->confirmarAccion(
      $id,
      'facturar',
      "¿Está seguro que desea enviar a hacienda la proforma número: " . $proformaNo . "?",
      'Después de confirmar la proforma será convertida en factura electrónica de compra',
      __('Sí, proceed')
    );
  }
}

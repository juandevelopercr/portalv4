...
class SellerManager extends BaseComponent
{
  use WithFileUploads;
  use WithPagination;
  ...
  {
    $records = Seller::search($this->search, $this->filters) // Utiliza el scopeSearch para la búsqueda
      ->orderBy($this->sortBy, $this->sortDir)
      ->paginate($this->perPage);

    return view('livewire.clasificadores.sellers.datatable', [
      'records' => $records,
    ]);
  }
  ...
  {
    $rules = [
      'name' => 'required|string|max:100|unique:sellers,name,' . $this->recordId . ',id',
  ...
  public function store()
  {
    ...
    try {
      ...
      $record = Seller::create($validatedData);
  ...
  public function update()
  {
    ...
    try {
      ...
      $record = Seller::findOrFail($recordId);
  ...
  public function refresDatatable()
  {
    $config = DataTableConfig::where('user_id', Auth::id())
      ->where('datatable_name', 'classifier-sellers-datatable')
      ->first();

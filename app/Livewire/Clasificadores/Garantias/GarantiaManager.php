...
class GarantiaManager extends BaseComponent
{
  use WithFileUploads;
  use WithPagination;
  ...
  {
    $records = Garantia::search($this->search, $this->filters) // Utiliza el scopeSearch para la búsqueda
      ->orderBy($this->sortBy, $this->sortDir)
      ->paginate($this->perPage);

    return view('livewire.clasificadores.garantias.datatable', [
      'records' => $records,
    ]);
  }
  ...
  {
    $rules = [
      'code'  => 'required|string|max:10|unique:garantias,code,' . $this->recordId . ',id',
  ...
  public function store()
  {
    ...
    try {
      ...
      $record = Garantia::create($validatedData);
  ...
  public function update()
  {
    ...
    try {
      ...
      $record = Garantia::findOrFail($recordId);
  ...
  public function refresDatatable()
  {
    $config = DataTableConfig::where('user_id', Auth::id())
      ->where('datatable_name', 'classifier-garantias-datatable')
      ->first();

...
class CaratulaManager extends BaseComponent
{
  use WithFileUploads;
  use WithPagination;
  ...
  {
    $records = Caratula::search($this->search, $this->filters) // Utiliza el scopeSearch para la búsqueda
      ->orderBy($this->sortBy, $this->sortDir)
      ->paginate($this->perPage);

    return view('livewire.clasificadores.caratulas.datatable', [
      'records' => $records,
    ]);
  }
  ...
  {
    $rules = [
      'code'  => 'required|string|max:10|unique:Caratulas,code,' . $this->recordId . ',id',
  ...
  protected function messages()
  {
    return [
      ...
      'code.max' => 'El código no debe superar los 20 caracteres.',
  ...
  public function store()
  {
    ...
    try {
      ...
      $record = Caratula::create($validatedData);
  ...
  public function update()
  {
    ...
    try {
      ...
      $record = Caratula::findOrFail($recordId);
  ...
  public function refresDatatable()
  {
    $config = DataTableConfig::where('user_id', Auth::id())
      ->where('datatable_name', 'classifier-caratulas-datatable')
      ->first();
